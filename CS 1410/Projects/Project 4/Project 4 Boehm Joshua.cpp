...
struct vacation
{
    string dest;
    string carr;
    float adult_fare, minor_fare;
    int adults, minors, all_passengers;
    float total_adult_fare, total_minor_fare, total_fare; 
};

struct vacation Vacay;

void calculation()
{
    // Variables to calculate and clarify adult passengers and costs
    Vacay.all_passengers = Vacay.adults + Vacay.minors;
    Vacay.total_adult_fare =  Vacay.adults * Vacay.adult_fare;
    Vacay.minor_fare = (Vacay.adult_fare * DISCOUNT);
    Vacay.total_minor_fare = Vacay.minor_fare * Vacay.minors;
    Vacay.total_fare = Vacay.total_adult_fare + Vacay.total_minor_fare;

    // Calculate ticket pricing depending on the party size of the user (based on inputs given)
    cout << left << setw(65) << "To recap: " << endl <<
            left << setw(65) << "You've entered that your desired destination is:" << right << setw(10) << Vacay.dest << endl <<
            left << setw(65) << "You've entered that your desired airline is:" << right << setw(10) << Vacay.carr << endl <<
            left << setw(65) << "You've entered that your airfare is:" << right << setw(10) << fixed << setprecision(2) << Vacay.adult_fare << endl <<
            left << setw(65) << "You've indicated your party size is:" << right << setw(10) << Vacay.all_passengers << endl <<
            left << setw(65) << "Adults in party:" << right << setw(10) << Vacay.adults << endl <<
            left << setw(65) << "Minors in party:" << right << setw(10) << fixed << setprecision(2) << Vacay.minors << endl <<
            left << setw(65) << "Ticket price for adults:" << right << setw(10) << fixed << setprecision(2) << Vacay.total_adult_fare << endl <<
            left << setw(65) << "Ticket price for minors:" << right << setw(10) << Vacay.total_minor_fare << endl <<
            left << setw(65) << "Total cost:" << right << setw(10) << fixed << setprecision(2) << Vacay.total_fare << endl;
}
...
void passengers()
{
    ...
    Vacay.adult_fare = airfare;
    Vacay.adults = total_passengers - minor_passengers;
    Vacay.minors = minor_passengers;
...
int main(){
    ...
    Vacay.dest = destination;

    // Display the destination that was chosen
    cout << "You selected " << Vacay.dest << "." << endl << endl;
